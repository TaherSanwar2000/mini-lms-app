...
describe('useDebounce', () => {
  ...
  it('resets timer when value changes mid-delay', () => {
    ...
    rerender({ v: 'end' });
    act(() => { jest.advanceTimersByTime(200); }); // only 200ms since 'end'
    expect(result.current).toBe('start'); // not updated yet
    act(() => { jest.advanceTimersByTime(100); }); // now 300ms total
    expect(result.current).toBe('end');
  });

  it('uses 300ms default delay when not specified', () => {
    const { result, rerender } = renderHook(
      ({ v }: { v: string }) => useDebounce(v),
